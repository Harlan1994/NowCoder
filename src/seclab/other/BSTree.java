...
public class BSTree {
    ...
    public TreeNode createBinaryTreeCore(TreeNode treeNode, char[] sequence, int current) {
        if (sequence[current] == ' ') {
            return treeNode;
        } else {
            TreeNode node = new TreeNode(Integer.valueOf(sequence[current]));
            node.leftChild = createBinaryTreeCore(treeNode, sequence, current + 1);
            node.rightChild = createBinaryTreeCore(treeNode, sequence, current + 1);
        }
        return treeNode;
    }


    public TreeNode createBinaryTree(char[] sequence) {
        if (sequence.length < 1) return null;

        TreeNode treeNode = null;
        for (int i = 0; i < sequence.length; i++) {
            treeNode = createBinaryTreeCore(treeNode, sequence, i);
        }
        return treeNode;
    }
